...
export function renderApp(state: AppViewState) {
  ...
  return html`
    <div class="shell ${isChat ? "shell--chat" : ""} ${chatFocus ? "shell--chat-focus" : ""} ${state.settings.navCollapsed ? "shell--nav-collapsed" : ""} ${state.onboarding ? "shell--onboarding" : ""}">
      ...
      state,
      onNext: () => { (state as any).onboardingStep++; },
      onPrev: () => { (state as any).onboardingStep--; },
      onComplete: async () => {
        (state as any).onboarding = false;
        // In a real app, we would send a POST to /api/user/onboarding here
        try {
          await fetch('/api/user/onboarding', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
              preferredChannel: state.settings.preferredChannel,
              enabledSkills: state.onboardingCategories
            })
          });
        } catch (e) {
