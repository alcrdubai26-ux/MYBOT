...
export async function runEmbeddedAttempt(
  ...
  // 1. Fetch multi-tenant environment overrides if userId is present
  let envOverrides = params.envOverrides ?? {};
  if (params.agentAccountId) {
    const { MultiTenantSkillManager } = await import("../../../../server/skill-manager.js");
    const dbEnv = await MultiTenantSkillManager.getSkillEnv(params.agentAccountId);
    envOverrides = { ...dbEnv, ...envOverrides };
  }
